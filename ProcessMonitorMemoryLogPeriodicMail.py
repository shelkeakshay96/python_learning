...
def MailSender(filename,time):
    try:
        ...
        toaddr = "akshay.test96@gmail.com"
...
def ProcessLog(log_dir = 'Marvellous'):
    ...
    logFileName = "MarvellousLog_%s.log" %(now)
    log_path = os.path.join(log_dir, logFileName)
    f = open(log_path, 'w')
    f.write(separator + "\n")
    txt = "Marvellous Infosystems Process Logger : "+time.ctime() + "\n"
    f.write(txt)
    f.write(separator + "\n")
    f.write("\n")
...
def main():
    ...
    try:
        schedule.every(float(argv[1])).minutes.do(ProcessLog)
        while True:
